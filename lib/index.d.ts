import CallServiceProvider from "./providers/CallServiceProvider";
import { useCallService } from "./providers/CallServiceProvider";
import backgroundMessageHandler from "./backgroundProcess/remoteMessage";
export default CallServiceProvider;
export { useCallService };
export { backgroundMessageHandler };
export type { SipCredentials, PushTokenInfo, SdkEvent, SipOptions, VoipSdkConfig, } from "./types/config";
