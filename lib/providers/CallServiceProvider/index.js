...
const CallServiceProvider = ({ children }) => {
    ...
    const startCall = (handle, name, calldata) => {
        ...
        if (!handle.startsWith('sip:') && react_native_1.Platform.OS === 'android') {
    ...
    const initiateCallService = async (token, isDev) => {
        await callService_1.default.start(token, isDev);
    };
    const enableAnalytics = (options) => {
        if (options.isEnabled) {
            callService_1.default.analyticsService.enableAnalytics(true);
            callService_1.default.analyticsService.identify(options.userId, options.properties);
        }
        else {
            callService_1.default.analyticsService.enableAnalytics(false);
            callService_1.default.analyticsService.resetAnalytics();
        }
    };
    ...
    const setPermissionsPrompts = (prompts) => {
        callService_1.default.setPermissionsPrompts(prompts);
    };
    (0, react_1.useEffect)(() => {
