...
class CallService extends eventemitter3_1.EventEmitter {
    ...
    async getAudioRecordPermission() {
        ...
        const hasPermission = await react_native_1.PermissionsAndroid.check(react_native_1.PermissionsAndroid.PERMISSIONS.AUDIO_RECORD);
        if (hasPermission) {
            return true;
        }
        react_native_1.Alert.alert('Permission required', 'To show native call screen and receive calls, enable our calling account on the next screen.', [
        ...
        const granted = await react_native_1.PermissionsAndroid.request(react_native_1.PermissionsAndroid.PERMISSIONS.AUDIO_RECORD);
        if (!granted) {
            return false;
        }
        return true;
    }
    async init(token, isDev) {
